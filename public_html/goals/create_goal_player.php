...
<?php	
// get player info from connected user since he must be the scorer (he can only upload goals scored by him)
$playerid = get_userplayerid($info["id"]);
?>
						<option value="<? echo $playerid; ?>" selected="selected"><? echo get_playername($playerid); ?></option>
					</select>
				</div>
...
<?
	...
	$result = mysql_query("SELECT * FROM players WHERE players.idplayers<>30 AND players.idplayers<>". $playerid. " AND players.clubid=". $clubid ) or die(mysql_error());
	$num=mysql_numrows($result);
	while($r = mysql_fetch_array( $result ))
		{
			//////////////////////////////////////////////////////
...
?> 
		...
		<br>
		<center><input type="button" value="[ Crear Gol ]" onclick="validate_form_goal_player()"></center>
